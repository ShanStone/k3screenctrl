...
uci show k3screenctrl > $TMP_DIR/k3_custom
...
do
	if [ -z "$(iptables -nvx -L FORWARD | grep DWSP | grep ${arp_ip[i]} -w)" -a -z "$(iptables -nvx -L FORWARD | grep UPSP | grep ${arp_ip[i]} -w)" ]; then
		...
		echo $(date +%s) > $TMP_DIR/device_speed/${arp_ip[i]}
		echo 0 >> $TMP_DIR/device_speed/${arp_ip[i]}
		echo 0 >> $TMP_DIR/device_speed/${arp_ip[i]}
	fi
done

if [ -s $TMP_DIR/lan_online_list.temp ]; then
	cat $TMP_DIR/lan_online_list.temp
	rm $TMP_DIR/lan_online_list.temp
	exit 0
else
	echo ${#online_list[@]}
fi
...
do
	...
	tmp_uci=$(cat $TMP_DIR/k3_custom | grep $tmp_mac | awk -F'=' '{print $1}' | awk -F'.' '{print$1"."$2}')
	if [ -n "$tmp_uci" ];then
		hostname[i]=$(uci get $tmp_uci.name)
		logo[i]=$(uci get $tmp_uci.icon)
	fi

	last_speed_time=$(cut -d$'\n' -f,1 $TMP_DIR/device_speed/${online_list[i]})
	last_speed_up=$(cut -d$'\n' -f,2 $TMP_DIR/device_speed/${online_list[i]})
	last_speed_dw=$(cut -d$'\n' -f,3 $TMP_DIR/device_speed/${online_list[i]})
	...
	echo $now_speed_time > $TMP_DIR/device_speed/${online_list[i]}
	echo $now_speed_up >> $TMP_DIR/device_speed/${online_list[i]}
	echo $now_speed_dw >> $TMP_DIR/device_speed/${online_list[i]}
...
if [ -s $TMP_DIR/arp_refresh_time ]; then
	last_arp_refresh_time=$(cat $TMP_DIR/arp_refresh_time)
	if [ $(($now_arp_refresh_time - $last_arp_refresh_time)) -ge 600 ]; then
		echo ${#online_list[@]} > $TMP_DIR/lan_online_list.temp
		for ((i=0;i<${#online_list[@]};i++))
		do
			arp -d ${online_list[i]}
			echo "${hostname[i]}" >> $TMP_DIR/lan_online_list.temp
			echo "${dw_sp[i]}" >> $TMP_DIR/lan_online_list.temp
			echo "${up_sp[i]}" >> $TMP_DIR/lan_online_list.temp
			echo "${logo[i]}" >> $TMP_DIR/lan_online_list.temp
		done
		echo 0 >> $TMP_DIR/lan_online_list.temp
		echo $now_arp_refresh_time > $TMP_DIR/arp_refresh_time
	fi
else
	echo $now_arp_refresh_time > $TMP_DIR/arp_refresh_time
fi
